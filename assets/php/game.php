        ...
        if (isset($_SESSION['player2'])) {
            ...
            if ($result = $db->query($sql))
                if (mysqli_num_rows($result) > 0) {
                    ...
                    if ($db->query($sql) === FALSE) $obj->message = $db->error . "rrr";
                    ...
                    if ($db->query($sql) === FALSE) $obj->message = $db->error . "ttt";
                    $obj->success = true;
                }
        } else if (isset($_SESSION['player1'])) {
            $_SESSION['in_game'] = true;
            $sql = "UPDATE LOGGED SET in_game = 1 WHERE username = '$username'";
            if ($db->query($sql) === FALSE) $obj->message = $db->error . "ppp";
            $obj->success = true;
        } else {
            $obj->message = "Pas d'adversaire ni de partie en cours</br>retour au menu principal";
            $_SESSION['in_game'] = false;
            $sql = "UPDATE LOGGED SET in_game = 0 WHERE username = '$username'";
            if ($db->query($sql) === FALSE) $obj->message = $db->error . "XD";
            $obj->success = false;
        }
                    ...
                    if ($row['player1_in_room'] == 1) {
                        ...
                        if ($row['score_player2'] == (intdiv($row['best_of'], 2) + 1) || $row['score_player1'] == (intdiv($row['best_of'], 2) + 1)) {
                            if ($row['score_player1'] > $row['score_player2']) {
                                ...
                                if ($db->query($sql) === FALSE) $obj->debug = $db->error . "caca";
                            } else {
                                $obj->message = "Victoire pour " . $row['player2'] . " !";
                                $obj->victory = true;
                                $sql = "UPDATE USERS SET score = score + 1 WHERE username = '" . $row['player2'] . "'";
                                if ($db->query($sql) === FALSE) $obj->debug = $db->error . "prout";
                            }
                            $sql = "DELETE FROM GAME WHERE id = '$id'";
                            if ($db->query($sql) === FALSE) $obj->debug = $db->error;
                            $sql = "UPDATE LOGGED SET in_game = 0 WHERE username = '" . $row['player2'] . "' OR username = '" . $row['player1'] . "'";
                            if ($db->query($sql) === FALSE) $obj->debug = $db->error . "pipi";
                        }
                        $obj->board = $board;
                    }
                } else {
                    ...
                    if ($row['player2_in_room'] == 1) {
                        ...
                        if ($row['score_player2'] == (intdiv($row['best_of'], 2) + 1) || $row['score_player1'] == (intdiv($row['best_of'], 2) + 1)) {
                            if ($row['score_player1'] > $row['score_player2']) {
                                ...
                                $obj->victory = true;
                                $sql = "UPDATE USERS SET score = score + 1 WHERE username = '" . $row['player1'] . "'";
                                if ($db->query($sql) === FALSE) $obj->debug = $db->error . "aze";
                            } else {
                                $obj->message = "Victoire pour " . $row['player2'] . " !";
                                $obj->victory = true;
                                $sql = "UPDATE USERS SET score = score + 1 WHERE username = '" . $row['player2'] . "'";
                                if ($db->query($sql) === FALSE) $obj->debug = $db->error . "eza";
                            }
                            $sql = "DELETE FROM GAME WHERE id = '$id'";
                            if ($db->query($sql) === FALSE) $obj->debug = $db->error;
                            $sql = "UPDATE LOGGED SET in_game = 0 WHERE username = '" . $row['player2'] . "' OR username = '" . $row['player1'] . "'";
                            if ($db->query($sql) === FALSE) $obj->debug = $db->error . "aaa";
                        }
                        $obj->board = $board;
                    }
                } else {
